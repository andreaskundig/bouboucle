...
const replaceRe = /<div id="([^"]+)"[^>]*> *(<img.+>)/g;
let index = 0;
while (matches = replaceRe.exec(htmlTemplate)) {
    const buttonId = matches[1];
    const stringToReplace = matches[2];
    index += 1;
    htmlTemplate = htmlTemplate.replace(stringToReplace, `\${buttonMap["${buttonId}"]}`);
}
